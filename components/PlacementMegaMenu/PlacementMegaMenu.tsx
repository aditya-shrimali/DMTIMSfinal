...
interface Props {
  setIsPlacementHovered: Dispatch<SetStateAction<boolean>>;
  isPlacementHovered: boolean;
}

const PlacementMegaMenu = ({
  setIsPlacementHovered,
  isPlacementHovered,
}: Props) => {
  ...
  return (
    <div
      onMouseEnter={() => setIsPlacementHovered(true)}
      onMouseLeave={() => setIsPlacementHovered(false)}
      className={`absolute hidden w-full bg-white px-4 lg:block ${
        isPlacementHovered ? "max-h-[50vh]" : "max-h-0"
            ...
            <Link legacyBehavior href={"/placements#Life"}>
              <a className="mb-1">
                <span
                  ...
                  className="link-underline link-underline-black pb-1 text-sm font-medium text-[#28314A] hover:text-[#4C477A]"
                >
                  111{" "}
                </span>
              </a>
...
export default PlacementMegaMenu;
